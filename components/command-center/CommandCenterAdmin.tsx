...
import { COMMAND_CENTER_WIDGETS, createWidget } from "@/lib/command-center/widget-registry";
...
export default function CommandCenterAdmin() {
  ...
  function addWidget(kind: CommandCenterWidgetKind) {
    if (!page) return;
    const nextWidget = placeWidget(page, createWidget(kind));
    if (!nextWidget) {
  ...
  async function perform(body: Record<string, unknown>, message: string) {
    ...
    try {
      ...
      if (result.token) setNewDeviceToken(result.token);
      await reload();
      setStatus("Gotowe");
      return result;
    } catch (actionError) {
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-6 text-slate-900 dark:bg-slate-950 dark:text-slate-100 sm:px-6">
      <div className="mx-auto max-w-[1800px]">
            ...
            <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
              ...
              {selectedDashboard && draft && page ? (
                <div className="grid min-h-[720px] lg:grid-cols-[210px_minmax(0,1fr)_240px]">
                  <aside className="border-r border-slate-200 p-3 dark:border-slate-700"><p className="mb-3 text-xs font-black uppercase tracking-wider text-slate-400">Biblioteka</p><div className="space-y-2">{COMMAND_CENTER_WIDGETS.map((definition) => <button draggable onDragStart={(event) => event.dataTransfer.setData("application/x-cc-widget-kind", definition.kind)} onClick={() => addWidget(definition.kind)} key={definition.kind} className="w-full cursor-grab rounded-xl border border-slate-200 p-3 text-left hover:border-sky-400 dark:border-slate-700"><strong className="block text-xs">{definition.name}</strong><span className="mt-1 block text-[10px] leading-snug text-slate-500">{definition.description}</span></button>)}</div></aside>
                  <div className="min-w-0 bg-slate-200/60 p-4 dark:bg-slate-950/60">
                    <div ref={previewRef} className="relative aspect-video w-full overflow-hidden rounded-xl bg-black shadow-2xl"><div style={{ height: 1080, transform: `scale(${previewScale})`, transformOrigin: "top left", width: 1920 }}><CommandCenterCanvas dashboardName={selectedDashboard.name} editing metrics={EMPTY_COMMAND_CENTER_METRICS} onLibraryDrop={(kind) => addWidget(kind as CommandCenterWidgetKind)} onSelectWidget={setSelectedWidgetId} onWidgetDrop={swapWidgets} page={page} selectedWidgetId={selectedWidgetId} theme="dark" /></div></div>
        ...
        {tab === "widgets" && <SimplePanel title="Biblioteka widżetów" subtitle="MVP zawiera wyłącznie metryki możliwe do policzenia z zarejestrowanych danych CRM."><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{COMMAND_CENTER_WIDGETS.map((item) => <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-700" key={item.kind}><strong>{item.name}</strong><p className="mt-2 text-sm text-slate-500">{item.description}</p><span className="mt-3 block text-xs text-slate-400">Domyślnie {item.defaultSize.w}×{item.defaultSize.h}</span></div>)}</div></SimplePanel>}
        {tab === "devices" && <DevicesPanel dashboards={data.dashboards} devices={data.devices} stations={data.stations} token={newDeviceToken} onCreate={(name, platform, dashboardId) => void perform({ action: "create-device", name, platform, dashboardId }, "Rejestrowanie urządzenia…")} onRotate={(deviceId) => void perform({ action: "rotate-device-token", deviceId }, "Generowanie nowego adresu urządzenia…")} onUpdate={(deviceId, field, value) => void perform({ action: "update-device", deviceId, [field]: value }, "Zapisywanie urządzenia…")} />}
